...
import { ArrowRight, Lock, LogIn, LogOut } from 'lucide-react';
...
export function AuditReport({
  ...
  const getSeverityData = (severity: string) => {
    switch (severity?.toLowerCase()) {
      case 'critical':
        return { tagClass: 'tag-crit', name: 'Critical', scoreCol: 'score-s1' };
      case 'high':
        return { tagClass: 'tag-high', name: 'High', scoreCol: 'score-s2' };
      case 'medium':
        return { tagClass: 'tag-med', name: 'Medium', scoreCol: 'score-s3' };
      case 'low':
        return { tagClass: 'tag-low', name: 'Low', scoreCol: 'score-s4' };
      default:
        return { tagClass: 'tag-cat', name: 'Unknown', scoreCol: 'score-s3' };
    }
  };

  const redirectParam = encodeURIComponent(`/results/${audit.id}`);

  return (
    <div className="report-wrapper" style={{ backgroundColor: '#F8F6F1', minHeight: '100vh', display: 'block' }}>
      {isLoggedIn && showAccountActions && (
      ...
      <div className="cover">
        <div className="cover-bar">
          <div className="cover-brand">Conversion Intelligence Report &nbsp;Â·&nbsp; Confidential</div>
          <div className="cover-date">Generated {formattedDate}</div>
        </div>
        <div className="cover-body">
          <div className="cover-eyebrow">CRO Audit â€” 12 Findings</div>
          <h1 className="cover-title">
            Where <span>{audit.url}</span>
            <br />
            is losing customers
          </h1>
          <div className="cover-client">
            <div className="cover-client-dot"></div>
            <div className="cover-client-name">{audit.url} &nbsp;Â·&nbsp; Conversion Analysis</div>
          </div>
        ...
        <div className="cover-footer">
          ...
          <div className="cover-footer-right">v1.0 &nbsp;Â·&nbsp; Page 1 of 1</div>
        </div>
      </div>

      <div className="page">
        <div className="section-eyebrow">Executive Summary</div>
        <h2 className="section-title">The short version â€” before the details</h2>
        ...
        <div className="score-section">
          ...
          <div className="chart-row">
            {[allResults.slice(0, Math.ceil(allResults.length / 2)), allResults.slice(Math.ceil(allResults.length / 2))].map((results, columnIndex) => (
              <div className="chart-card" key={columnIndex}>
                <div className="chart-label">{columnIndex === 0 ? 'Scores by dimension' : '\u00a0'}</div>
                    ...
                    return (
                      <div className="sbar-row" key={idx}>
                        ...
                        <div className="sbar-track">
                          <div className="sbar-fill" style={{ width: `${res.score * 10}%`, background: colorMap[sd.scoreCol] ?? 'var(--muted)' }}></div>
                        </div>
        ...
        <h2 className="section-title">
          {isLoggedIn ? 'All 12 Issues â€” Full Report' : `4 of ${allResults.length} Issues â€” Free Preview`}
        </h2>
              ...
              <div className="finding-card" key={result.id || index}>
                <div className="finding-header">
                  ...
                  <div className="finding-header-body">
                    <div className="finding-tags">
                      ...
                      <span className="tag tag-cat">{result.category || 'UX Â· Conversion'}</span>
                    </div>
                ...
                <div className="finding-body">
                  <div className="finding-col">
                    <div className="finding-col-label">UX Analysis</div>
                    <p>{result.problem}</p>
                    {result.example && (
                  ...
                  <div className="finding-col">
                    <div className="finding-col-label">Recommended Action</div>
                    <p>{result.fix}</p>
                    {result.impact && (
          ...
          <div className="mt-16">
            <div className="relative overflow-hidden rounded-xl border border-[var(--border)] bg-[#fff]">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-[var(--paper)]/85 backdrop-blur-[3px] p-6 text-center">
                ...
                <h3 className="text-2xl font-serif text-[var(--ink)] mb-2">{lockedResults.length} More Issues Found</h3>
                <p className="text-[var(--muted)] max-w-md mx-auto mb-2">
                  You&apos;ve seen 4 of {allResults.length} conversion blockers. Sign up free to unlock the full report instantly â€” no credit card required.
                </p>
